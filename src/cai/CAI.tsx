...
const CaiMessageView = ({ message }: { message: cai.CaiMessage }) => {
    ...
    const wholeMessage = message.text.map((phrase: [string, string], index) => {
        switch (phrase[0]) {
            ...
            case "LINK":
                return <a key={index} className="hover:text-yellow-500 text-blue-500 underline" href="#" onClick={e => { e.preventDefault(); /* Disabled for STEM Day app: dispatch(curriculum.open(phrase[1][1])); */ addToNodeHistory(["curriculum", phrase[1][1]]) }}>{phrase[1][0]}</a>
            case "sound_rec":
